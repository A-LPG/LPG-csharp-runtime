...
namespace LPG2.Runtime
{
    ...
    public abstract class Differ
    {
        ...
        {
            int end = start + len;
            if (buf == null || len <= 0 || buf.Length > end)
            {
                return "";
            }
      ...
      public  class Change
        ...
        const  int M_CODE = 3;
        const  int MM_CODE = 4;
        const  int IMI_CODE = 5;
        const  int MI_CODE = 6;
        const  int IM_CODE = 7;
        const  int DMD_CODE = 8;
        const  int MD_CODE = 9;
        const  int DM_CODE = 10;

      protected  IPrsStream newStream,
            oldStream;
      ...
      public ILine[] newBuffer,
            oldBuffer;

      private Dictionary<ILine,int> newMap = new Dictionary<ILine, int>();
        public int[] newLink;
        ...
        public Differ(IPrsStream newStream, IPrsStream oldStream)
        {
            ...
            for (i = 1; i < newBuffer.Length; i++)
            {
                ...
                int root = 0;
                if (!newMap.TryGetValue(line,out root))
                {
                    newMap[line]=i;
                    root = i;

                }
                else newLink[tail[root]] = i;
        ...
        public  int getReplaceDeleteCount() {
            return replaceDeleteCount;
        }

        public  int getReplaceInsertCount() {
            return replaceInsertCount;
        }
        ...
        public void outputChanges()
        {
            for (Change element = changeRoot; element != null; element = element.getNext())
            {
                if (element.getCode() == I_CODE)
                    ...
                    Console.Out.WriteLine("Don't know what to do with code " + element.getCode());
            }
        ...
        //
        void detach(ILine []buffer, Dictionary<ILine,int> map, int[] link, int start, int end)
        {
            for (int k = start; k <= end; k++)
            {
                ILine line = buffer[k];
                int root = 0;// (Integer) map.get(line);
                if (map.TryGetValue(line, out root))
                {
                    int previous = 0;
        ...
        void addReplace(int olds, int olde, int news, int newe)
        {
            ...
            Change element = replaceRoot, tail = null;
            for (;
                element != null;
                tail = element, element = element.getNext())
            {
                ...
                if (bound1 == (newe - news) && bound2 == (olde - olds))
                {
                    if (compareSections(element.getOlds(), news, bound1))
                    {
                        ...
                        {
                            Change change_element = new Change(M_CODE, element.getOlds(), element.getOlde(), news, newe,
                                changeCount);
            ...
            element = new Change(R_CODE, olds, olde, news, newe, changeCount);
        ...
        void findMoves()
        {
            ...
            //
            for (i_element = insertRoot, previous_i = null;
                i_element != null && deleteRoot != null;
                previous_i = i_element, i_element = i_element.getNext())
            {
                int bound = i_element.getNewe() - i_element.getNews();

                for (d_element = deleteRoot, previous_d = null;
                    d_element != null;
                    previous_d = d_element, d_element = d_element.getNext())
                {
        ...
        void compare(int olds, int olde, int news, int newe)
        {
            ...
            else // Find largest subsection that matches
            {
                ...
                {
                    int image = 0; // list of OLDI lines
                    if (newMap.TryGetValue(oldBuffer[oldi], out image))
                    {
        ...
        {
            Dictionary<ILine,int> map = new Dictionary<ILine, int>();
            ...
            bound = 0;
            int  image ;
            if (map.TryGetValue(oldBuffer[d_element.getOlds()], out image))
            {
        ...
        bool insertOverlap(Change d_element, Change i_element)
        {
            ...
            int image ;
            if (map.TryGetValue(oldBuffer[i_element.getNews()], out image))
            {
        ...
        void mergeChanges()
        {
            ...
            bool []slot_used = new bool[changeCount + 1];
     ...
     public   abstract void outputInsert(Change element);
     public abstract void outputDelete(Change element);
     public abstract void outputReplace(Change element);
     public abstract void outputMove(Change element);
     public abstract void outputMoveDelete(Change element);
     public abstract void outputMoveInsert(Change element);
    }
}