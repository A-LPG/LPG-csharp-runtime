...
namespace LPG2.Runtime
{
    ...
    public class DiagnoseParser : ParseErrorCodes
    {
        ...
        //
        //
        public DiagnoseParser(TokenStream tokStream, ParseTable prs) :this(null, tokStream, prs)
        {
        ...
        protected  const int STACK_INCREMENT = 256;

        protected  const int BUFF_UBOUND = 31,
            BUFF_SIZE = 32,
            MAX_DISTANCE = 30,
            MIN_DISTANCE = 3;
        ...
        protected class SecondaryRepairInfo
        {
            ...
            public bool recoveryOnNextStack;
        }
        ...
        protected virtual void reallocateStacks()
        {
            ...
            else
            {
                ...
                Array.Copy(tempStack, 0, tempStack = new int[stack_length], 0, old_stack_length);
                Array.Copy(prevStack, 0, prevStack = new int[stack_length], 0, old_stack_length);
                Array.Copy(nextStack, 0, nextStack = new int[stack_length], 0, old_stack_length);
                Array.Copy(scopeIndex, 0, scopeIndex = new int[stack_length], 0, old_stack_length);
                Array.Copy(scopePosition, 0, scopePosition = new int[stack_length], 0, old_stack_length);
            }
        ...
        public void diagnoseEntry(int marker_kind, int error_token)
        {
            ...
            Array.Copy(tempStack, 0, stateStack, 0, tempStackTop + 1);
            ...
            do
            {
                ...
                Array.Copy(stateStack, 0, tempStack, 0, stateStackTop + 1);
        ...
        protected RepairCandidate primaryPhase(int error_token)
        {
            ...
            if (repair.bufferPosition == 1)
            {
                ...
                Array.Copy(prevStack, 0, stateStack, 0, stateStackTop + 1);
            }
            else if (nextStackTop >= 0 && repair.bufferPosition >= 3)
            {
                stateStackTop = nextStackTop;
                Array.Copy(nextStack, 0, stateStack, 0, stateStackTop + 1);
        ...
        protected int mergeCandidate(int state, int buffer_position)
        {
            ...
            for (int k = asi(state); asr(k) != 0; k++)
            {
                ...
                {
                    if (str.ToLower().Equals(name(i).ToLower()))
                        return asr(k);
                }
            }
        ...
        protected void scopeTrialCheck(PrimaryRepairInfo repair, int[] stack, int stack_top, int indx)
        {
            ...
            if (statePoolTop >= statePool.Length)
                Array.Copy(statePool, 0, statePool = new StateInfo[statePoolTop * 2], 0, statePoolTop);
                ...
                for (int action_index = 0; action_index < action.size(); action_index++)
                {
                    ...
                    if (act != ERROR_ACTION)
                    {
                        ...
                        for (j = tempStackTop + 1;
                            j >= (max_pos + 1) &&
                            inSymbol(tempStack[j]) == scopeRhs(k);
                            j--)
                            k++;
        ...
        //
        protected bool secondaryCheck(int[] stack, int stack_top, int buffer_position, int distance)
        {
        ...
        //
        protected void misplacementRecovery(SecondaryRepairInfo repair, int []stack, 
            int stack_top, int last_index, bool
            stack_flag)
        {
        ...
        //
        protected void secondaryRecovery(SecondaryRepairInfo repair, int []stack, 
        int stack_top, int last_index, bool
            stack_flag)
        {
            int previous_loc = buffer[2],
