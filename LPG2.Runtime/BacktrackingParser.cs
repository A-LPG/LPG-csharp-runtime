...
public class BacktrackingParser : Stacks
{
    ...
    private bool skipTokens = false; // true if error productions are used to skip tokens
    ...
    public  int getCurrentRule()     { return currentAction; }
    public  int getFirstToken()      { return tokStream.getFirstRealToken(getToken(1)); }
    public  int getFirstToken(int i) { return tokStream.getFirstRealToken(getToken(i)); }
    public  int getLastToken()       { return tokStream.getLastRealToken(lastToken); }
    public  int getLastToken(int i)  { int l = (i >= prs.rhs(currentAction)
                                                        ? lastToken
    ...
    public BacktrackingParser(TokenStream tokStream, ParseTable prs, RuleAction ra)
    {
        reset(null, tokStream, prs, ra);
    }

    public BacktrackingParser(Monitor monitor, TokenStream tokStream, ParseTable prs, RuleAction ra) 
    {
    ...
    {
        // assert(base.stateStack != null);
        if (this.actionStack == null)
        {
            this.actionStack = new int[base.stateStack.Length];
            base.locationStack = new int[base.stateStack.Length];
            base.parseStack = new object[base.stateStack.Length];

            actionStack[0] = 0;
            locationStack[0] = start_token_index;
        }
        else if (this.actionStack.Length < base.stateStack.Length)
        {
            int old_length = this.actionStack.Length;

            Array.Copy(this.actionStack, 0, this.actionStack = new int[base.stateStack.Length], 0, old_length);
            Array.Copy(base.locationStack, 0, base.locationStack = new int[base.stateStack.Length], 0, old_length);
            Array.Copy(base.parseStack, 0, base.parseStack = new object[base.stateStack.Length], 0, old_length);
        }
        return;
    }
    ...
    //
    public object fuzzyParse() 
    {
        return fuzzyParseEntry(0, int.MaxValue);
    }

    //
    // Recover up to max_error_count times and then quit
    //
    public object fuzzyParse(int max_error_count) 
    {
    ...
    // Always attempt to recover
    //
    public object fuzzyParseEntry(int marker_kind) 
    {
        
        return fuzzyParseEntry(marker_kind, int.MaxValue);
    }

    //
    //
    //
    public object fuzzyParseEntry(int marker_kind, int max_error_count)
    {
        action.reset();
    ...
    public object parseEntry(int marker_kind, int max_error_count) 
    {
        ...
        if (max_error_count > 0 && tokStream is IPrsStream)
            max_error_count = 0;
        ...
        Array.Copy(stateStack, 0, temp_stack, 0, temp_stack.Length);
             ...
             error_token = backtrackParse(action, repair_token), count++)
        {
            ...
            Array.Copy(temp_stack, 0, stateStack, 0, temp_stack.Length);
    ...
    //
    private  void process_reductions()
    {
        do
        {
...
//System.err.println("Goto state " + prs.originalState(currentAction));
    ...
    //
    private object parseActions(int marker_kind) 
    {
        int ti = -1,
            curtok;
...
//System.err.println("reducing by rule " + currentAction);              
            ...
            else                   // a shift or shift-reduce action
            {
                if (tokStream.getKind(curtok) > NT_OFFSET) 
                {
                    ...
                    ProstheticAst prosthesis = null;
                    if (prostheticAst != null)
                    {
                        int slot = prs.getProsthesisIndex(tokStream.getKind(curtok));
                    ...
                    parseStack[stateStackTop] = prosthesis(((PrsStream) tokStream).getIToken(curtok));
                }
                lastToken = curtok;
                curtok = tokens.get(++ti);
...
//else System.err.println("Shifting on token " + tokStream.getName(lastToken) + " to state " + prs.originalState(currentAction));              
    ...
    private int process_backtrack_reductions(int act)
    {
        ...
        //
    public int backtrackParse(int[] stack, int stack_top, IntSegmentedTuple action, int initial_token)
    {
        stateStackTop = stack_top;
        Array.Copy(stack, 0, stateStack, 0, stateStackTop + 1);
    ...
    //
    public int backtrackParse(IntSegmentedTuple action, int initial_token)
    {
        //
        ...
        // Console.Out.WriteLine("****Max Number of configurations: " + configuration_stack.maxConfigurationSize());
        // for (ConfigurationElement configuration = configuration_stack.pop();
        //      configuration != null;
        //      configuration = configuration_stack.pop())
//        {
//Console.Out.WriteLine("    restart at position " + configuration.action_length +
...
//        }
//
        // Console.Out.WriteLine("****Number of elements in stack tree: " + configuration_stack.numStateElements());
        // Console.Out.WriteLine("****Number of elements in stacks: " + configuration_stack.stacksSize());
        // Console.Out.WriteLine("****Number of actions: " + action.size());
        // Console.Out.WriteLine("****Max Stack Size = " + maxStackTop);
        // System.out.flush();
    ...
    private bool repairable(int error_token)
    {
        //
        // Allocate configuration stack.
        //
    ...
    private bool recoverableState(int state)
    {
